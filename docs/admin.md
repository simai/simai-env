...
- Site doctor: read-only diagnostics against profile (filesystem, nginx, PHP, cron, SSL, DB) with PASS/WARN/FAIL summary; for Bitrix it also warns about root-owned docroot files, fails when nginx SEF fallback does not point to `/bitrix/urlrewrite.php`, and warns when static asset compression/cache headers are missing. It does not apply changes.
