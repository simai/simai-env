...
Profiles are declarative files under `profiles/<id>.profile.sh` and are consumed by simai-admin to drive site lifecycle. See `docs/developer/architecture/profiles.md` and `docs/developer/architecture/profiles-spec.md` for background and field definitions.
...
PROFILE_ALLOWED_PHP_MIN_VERSION=""
PROFILE_ALLOWED_PHP_MAX_VERSION=""
...
- PHP: `PROFILE_ALLOWED_PHP_VERSIONS` or `PROFILE_ALLOWED_PHP_MIN_VERSION`/`PROFILE_ALLOWED_PHP_MAX_VERSION`, `PROFILE_PHP_EXTENSIONS_REQUIRED/RECOMMENDED`, `PROFILE_PHP_INI_REQUIRED/RECOMMENDED/FORBIDDEN`
