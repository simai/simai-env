...
In the interactive menu, the daily WordPress actions appear inside `Applications -> WordPress`.

All commands require a site with `wordpress` profile.

Run with `sudo /root/simai-env/simai-admin.sh wp <command> [options]`.

Use this doc when you need:

- WordPress lifecycle and installer preparation,
- managed cron and cache behavior,
- WordPress-specific optimization and finalize workflows.
