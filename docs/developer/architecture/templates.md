...
Use this document when you need the managed rendering surfaces used by installer and admin flows.

Why it matters:

- generated nginx, healthcheck, and queue artifacts come from these templates,
- profile behavior depends on which template is selected,
- safe config rewrites rely on placeholder conventions staying stable.
...
- [architecture/profiles.md](./profiles.md)
- [architecture/nginx-metadata.md](./nginx-metadata.md)
