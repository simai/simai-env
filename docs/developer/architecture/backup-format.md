...
Use this document when you need the exact structure and safety contract of `backup export` / `backup import` archives.

Why it matters:

- backup archives are config-only, not full site backups,
- import behavior depends on archive structure and profile compatibility,
- this file is the source of truth for what is intentionally excluded from the archive.
