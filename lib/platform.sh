...
platform_print_os_support_status() {
  ...
  if platform_is_supported_os; then
    ...
    if platform_has_tty; then
      printf "[ERROR] %sOS: %s (%s %s %s)%s — unsupported. Supported: %s\n" "${RED}" "${PLATFORM_OS_PRETTY}" "${PLATFORM_OS_ID}" "${PLATFORM_OS_VERSION_ID}" "${PLATFORM_OS_CODENAME}" "${RESET}" "${supported_matrix}" >/dev/tty
    else
      printf "[ERROR] OS: %s (%s %s %s) — unsupported. Supported: %s\n" "${PLATFORM_OS_PRETTY}" "${PLATFORM_OS_ID}" "${PLATFORM_OS_VERSION_ID}" "${PLATFORM_OS_CODENAME}" "${supported_matrix}"
    fi
    return 1
  fi
}
