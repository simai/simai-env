    ...
    while true; do
      ...
      if [[ $show_advanced -eq 1 ]]; then
        items=(
          ...
          "4|Activity & optimization"
          "5|Change activity class"
          "6|Optimization override status"
          "7|Turn auto optimization off"
          "8|Turn auto optimization on"
          "9|Reset optimization override"
  ...
  laravel_menu() {
    while true; do
      ...
      case "$ch" in
        ...
        15) run_menu_command bitrix agents-status ;;
        16) run_menu_command bitrix agents-sync ;;
        17) run_menu_command bitrix db-preseed ;;
        18) run_menu_command bitrix installer-ready ;;
        19)
          if [[ $show_advanced -eq 1 ]]; then
