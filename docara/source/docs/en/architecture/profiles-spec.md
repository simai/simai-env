...
- PHP runtime: `PROFILE_ALLOWED_PHP_VERSIONS` or `PROFILE_ALLOWED_PHP_MIN_VERSION`/`PROFILE_ALLOWED_PHP_MAX_VERSION`, `PROFILE_PHP_EXTENSIONS_REQUIRED/RECOMMENDED/OPTIONAL`, `PROFILE_PHP_INI_REQUIRED/RECOMMENDED/FORBIDDEN`
...
- For PHP version constraints, explicit `PROFILE_ALLOWED_PHP_VERSIONS` has priority. When that array is empty, `PROFILE_ALLOWED_PHP_MIN_VERSION` / `PROFILE_ALLOWED_PHP_MAX_VERSION` are used as dynamic bounds.
