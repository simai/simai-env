...
Examples:
  ...
  simai-env.sh clean --project-name blog --domain example.com --remove-files --drop-db
USAGE
}
...
create_database() {
  ...
  mysql -uroot -e "GRANT ALL PRIVILEGES ON \`${DB_NAME}\`.* TO '${DB_USER}'@'%'; FLUSH PRIVILEGES;" >>"$LOG_FILE" 2>&1 || warn "Failed to grant privileges"
}
...
configure_cron() {
  local cron_line="* * * * * ${PHP_BIN:-/usr/bin/php} ${PROJECT_PATH}/artisan schedule:run >> /dev/null 2>&1"
  local tmp
  tmp=$(mktemp)
  crontab -u "$SIMAI_USER" -l 2>/dev/null | grep -v "${PROJECT_PATH}/artisan schedule:run" > "$tmp" || true
  echo "$cron_line" >> "$tmp"
  crontab -u "$SIMAI_USER" "$tmp"
  rm -f "$tmp"
}
